...
const supportedPageVisibilityEventName: string = ((): string => {
  ...
  const supported: string | undefined = find(candidates, (eventName: string): boolean => `on${eventName}` in document)
...
type NewType = (event: KeyboardEvent & TouchEvent & EventListenerOrEventListenerObject) => void
...
function bindEvents(el: HTMLElement | Window, bindings: EventBinding[], sharedOptions?: EventOptions) {
  const unbindings: UnbindFn[] = bindings.map((binding: EventBinding): UnbindFn => {
    ...
    el.addEventListener(binding.eventName, binding.fn as EventListenerOrEventListenerObject, options)

    return function unbind() {
      el.removeEventListener(binding.eventName, binding.fn as EventListenerOrEventListenerObject, options)
    }
  })
  ...
  const startCaptureBinding = useMemo(
    () => ({
      ...
      fn: function onTouchStart(event: TouchEvent) {
        ...
        const handle: HTMLElement | null = document.querySelector(`[data-rbd-drag-handle-draggable-id='${dragHandleId}']`)
        if (!handle) {
