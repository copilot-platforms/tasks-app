...
export const AssigneeFetcher = async ({ token, userType }: Props) => {
  ...
  const AssigneeSuggestions = assignableUsersWithType.map((item) => ({
    id: item.id,
    label: item?.name ?? `${item.givenName} ${item.familyName}`,
  }))

  return <ClientSideStateUpdate assignee={assignableUsersWithType} assigneeSuggestions={AssigneeSuggestions} />
}
